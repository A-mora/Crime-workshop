...
# 3.Getting starter with R Studio  
...
For example, n my PC will look like this (it won't work in your PC because the path s different)

```{r, eval=FALSE}
setwd("C:/Users/Anita/UKDS/NCRM workshop")
```

In cases when you are unsure about where is your current working directory, you can use the function `getwd()`, which literally means: "get working directory". The working directory will be printed in your console.
...
Installing packages can be done by code of clicking in the "Tools" tab of the R Studio bar. You only need to install packages once (unless you change computer).
...
Creating variables (vectors) of length 1, using the assignation indicator
```{r}
a <- 3
b <- 5
c <- 9
...
The function we use to import data into R will depend of the format in which the data is. Data in `.txt` or `.csv` can be installed using base R functions (don't need to use any package), but data in Stata, SPSS or SAS format, to name a few, need a special package.

Today we will be using the the package [readr](https://cran.r-project.org/web/packages/readr/readr.pdf) which is part of the `tidyverse` family, to import delimited files, such as txt and csv or excel files. 
...
Each of this packages have a specific function to read a specific dataset. You can check the package documentation (linked above) or a simple Google search to get your answer.

In this example we used the function `read_csv` of the package 'readr' to read a  .csv files. The data will be loading is g open data from the Police crime record for Manchester. This data can be accessed on the [police.uk](https://data.police.uk/data/) website. 
```{r}
# change name to the file, copying into the folder
...
If you want to have a look to more examples on how to use pipes, you can check chapter 18 of the free book [R for data Science](https://r4ds.had.co.nz/pipes.html)
